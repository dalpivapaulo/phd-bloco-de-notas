...
function getAlertPref(id){
  const p=loadAlertPrefs()[String(id)]||{};
...
async function prepareNativeAlertPermissions(){
  ...
  try{
    ...
    if(!status?.batteryUnrestricted){
      toast("Para o PHD avisar mesmo com o app fechado, autorize o uso sem restrição de bateria.");
      await api.requestBatteryUnrestricted();
      return;
    }
...
function openNew(){
  ...
  $("remNotify").checked=false;$("remWakeScreen").checked=false;syncAlertControls();
  $("deleteBtn").classList.add("hidden");$("doneBtn").classList.add("hidden");$("dialog").showModal();
}
function edit(id){
  ...
  const alertPref=getAlertPref(r.id);$("remNotify").checked=alertPref.notify_enabled;$("remWakeScreen").checked=alertPref.wake_screen;syncAlertControls();
  $("deleteBtn").classList.remove("hidden");$("doneBtn").classList.remove("hidden");$("doneBtn").textContent=r.status==="done"?"Reabrir":"Concluir";$("dialog").showModal();
}
...
function initVoice(){
  ...
  btn.onclick=()=>{
    ...
    if(!SR){
      toast("Ditado automático indisponível. Use o microfone do teclado.");
      return;
    }
    ...
    btn.classList.add("listening");btn.textContent="⏹️";if(status)status.textContent="Ouvindo...";
    speechRecognition.onresult=e=>{
      let spoken="";
      for(let i=0;i<e.results.length;i++)spoken+=e.results[i][0].transcript;
      spoken=spoken.trim();
      if(!base)spoken=capFirst(spoken);
    ...
    speechRecognition.onend=()=>{
      btn.classList.remove("listening");btn.textContent="🎙️";if(status)status.textContent="";speechRecognition=null;
    };
    try{speechRecognition.start()}catch{
      btn.classList.remove("listening");btn.textContent="🎙️";if(status)status.textContent="";speechRecognition=null;toast("Não foi possível iniciar o microfone.");
    }
  };
}
